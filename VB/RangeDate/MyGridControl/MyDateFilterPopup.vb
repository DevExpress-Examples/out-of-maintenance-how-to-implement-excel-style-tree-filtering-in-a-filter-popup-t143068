...
Namespace DateRange
	...
	Public Class MyDateFilterPopup
		...
		Protected Overridable Function GetBetweenOperatorByName(ByVal node As TreeListNode) As BetweenOperator
			If node.Level = 0 Then
				Dim firstDay As New Date(DirectCast(node.GetValue(treelist.Columns(0)), Integer), 01, 01)
				Dim endDay As New Date(DirectCast(node.GetValue(treelist.Columns(0)), Integer), 12, 31)
				Return New BetweenOperator(Me.Column.FieldName, firstDay, endDay)
			ElseIf node.Level = 1 Then
				Dim firstDay As New Date(DirectCast(node.ParentNode.GetValue(treelist.Columns(0)), Integer), GetMonth(node), 01)
				Dim endDay As New Date(DirectCast(node.ParentNode.GetValue(treelist.Columns(0)), Integer), GetMonth(node), Date.DaysInMonth(DirectCast(node.ParentNode.GetValue(treelist.Columns(0)), Integer), GetMonth(node)))
				Return New BetweenOperator(Me.Column.FieldName, firstDay, endDay)
			Else
				Dim firstDay As New Date(DirectCast(node.ParentNode.ParentNode.GetValue(treelist.Columns(0)), Integer), GetMonth(node.ParentNode), DirectCast(node.GetValue(treelist.Columns(0)), Integer))
				Dim endDay As New Date(DirectCast(node.ParentNode.ParentNode.GetValue(treelist.Columns(0)), Integer), GetMonth(node.ParentNode), DirectCast(node.GetValue(treelist.Columns(0)), Integer))
				Return New BetweenOperator(Me.Column.FieldName, firstDay, endDay)
			End If
		End Function
		Private Function GetMonth(ByVal node As TreeListNode) As Integer
			Dim dt As Date = Date.ParseExact(DirectCast(node.GetValue(treelist.Columns(0)), String), "MMMM", CultureInfo.InvariantCulture)
			Return dt.Month
		End Function
	End Class
End Namespace